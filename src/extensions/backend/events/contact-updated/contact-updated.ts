...
export default contacts.onContactUpdated(async (event: any) => {
  ...
  const contact =
    event.entity ?? event.updatedEvent?.currentEntity ?? event.data?.entity;
  if (!contact) {
  ...
  if (
    contact.source?.sourceType === "WIX_FORMS" &&
    (contact.revision ?? 0) <= 2
  ) {
    return;
  }

  try {
    const { WixContactsService } =
      await import("../../../../backend/services/wix-contacts.service");
    const { TokenManager } =
