...
export class HubSpotService {
  ...
  async getContactProperties(): Promise<any[]> {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to get contact properties: ${response.statusText}`,
      );
    }
  ...
  async createOrUpdateContact(
    ...
    try {
      ...
      if (
        ...
        errMsg.includes("PROPERTY_DOESNT_EXIST")
      ) {
        ...
        if (customProps.length > 0) {
          logger.info(
            "Attempting to create missing HubSpot custom properties",
          ...
          try {
            ...
            logger.warn(
              "Retry with custom properties failed, falling back to standard only",
              {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (response.status !== 409) {
            logger.warn("Failed to create HubSpot property", {
              name,
              status: response.status,
              err,
            });
